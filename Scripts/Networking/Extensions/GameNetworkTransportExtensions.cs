    ...
    public static partial class GameNetworkTransportExtensions
    {
        private static void Send(LiteNetLibManager.LiteNetLibManager manager, long? connectionId, ushort msgType, INetSerializable message)
        {
            if (!connectionId.HasValue)
                manager.ClientSendPacket(5, DeliveryMethod.ReliableOrdered, msgType, message.Serialize);
            else
                manager.ServerSendPacket(connectionId.Value, 5, DeliveryMethod.ReliableOrdered, msgType, message.Serialize);
        }

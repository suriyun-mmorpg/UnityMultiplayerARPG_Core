...
namespace MultiplayerARPG
{
    public partial class AnimatorCharacterModel2D : BaseCharacterModel
    {
        ...
        protected override void OnValidate()
        {
            ...
            if (animator == null)
                Logging.LogError(ToString(), $"{name}(AnimatorCharacterModel2D) -> `Animator` is empty, please set reference of `Animator` component to `AnimatorCharacterModel2D` -> `Animator` field.");
            if (animatorController == null)
                Logging.LogError(ToString(), $"{name}(AnimatorCharacterModel2D) -> `AnimatorController` is empty, please set reference of `AnimatorController` data to `AnimatorCharacterModel2D` -> `AnimatorController` field.");
            if (hasChanges)
            {

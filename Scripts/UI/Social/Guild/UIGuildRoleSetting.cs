...
namespace MultiplayerARPG
{
    public class UIGuildRoleSetting : UIBase
    {
        ...
        public void OnClickSetting()
        {
            ...
            {
                UISceneGlobal.Singleton.ShowMessageDialog(LanguageManager.GetText(UILocaleKeys.UI_LABEL_WARNING.ToString()), LanguageManager.GetText(UILocaleKeys.UI_GUILD_ROLE_NAME_IS_EMPTY.ToString()));
                return;
            }
            if (inputFieldShareExpPercentage == null || 
                !byte.TryParse(inputFieldShareExpPercentage.text, out shareExpPercentage))
            {
                UISceneGlobal.Singleton.ShowMessageDialog(LanguageManager.GetText(UILocaleKeys.UI_LABEL_WARNING.ToString()), LanguageManager.GetText(UILocaleKeys.UI_GUILD_ROLE_SHARE_EXP_NOT_NUMBER.ToString()));
                return;
            }
